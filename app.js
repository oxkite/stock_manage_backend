...
async function getDiffProducts(id, getHigher = 0) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (category === "0") {
      for (cat of Categories) {
        if (cat.id !== 0) {
          let products = await getDiffProducts(cat.id);
      ...
      let products = await getDiffProducts(category);
      updateProductPrices(products, percent).then((newResult) => {
...
app.get("/:id/:higher", async (req, res) => {
  ...
  if (req.params.id == 0) {
    for (cat of Categories) {
      if (cat.id !== 0) {
        const products = await getDiffProducts(cat.id, req.params.higher);
        result = result.concat(products);
      }
    }
  } else {
    console.log(req.params.higher);
    result = await getDiffProducts(req.params.id, req.params.higher);
  }
  //console.log(result);
...
app.get("/setHigh", async function (req, res) {
  ...
  for (cat of Categories) {
    if (cat.id !== 0) {
      const products = await getDiffProducts(cat.id, "0");
      result = result.concat(products);
    }
  }
  for (res of result) {
